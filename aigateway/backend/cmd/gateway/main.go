...
func main() {
	...
	modelSvc := service.NewModelService(modelRepo, bindingRepo, providerRepo, modelPricingRepo, userModelPermRepo, appLogger)
	...
	imageSvc := service.NewImageService(
		modelRepo, bindingRepo, providerRepo, routerSvc,
		billingSvc, policySvc, usageSvc, logRepo, httpClient, appLogger,
	)
	...
	adminUserCtrl := controller.NewAdminUserController(adminUserSvc, rbacSvc, appLogger)
	...
	protectedMux.HandleFunc("PUT /api/v1/admin/users/{id}/password", adminUserCtrl.HandleResetPassword)
