...
const (
	userColumns = "id, organization_id, email, nickname, user_status, password_hash, plain_password, role_id, quota_balance, created_at, updated_at, deleted_at"
)
...
func (r *PostgresUserRepository) Create(ctx context.Context, user *entity.User) error {
	query := `INSERT INTO users (email, nickname, password_hash, plain_password, user_status, role_id, quota_balance, created_at, updated_at)
		VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
		RETURNING id, created_at, updated_at`

	now := time.Now()
	err := r.pool.QueryRow(ctx, query,
		user.Email, user.Nickname, user.PasswordHash, user.PlainPassword, user.UserStatus, user.RoleID, user.QuotaBalance, now, now,
