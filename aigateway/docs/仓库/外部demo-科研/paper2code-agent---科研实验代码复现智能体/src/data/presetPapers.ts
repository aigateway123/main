...
export const PRESET_PAPERS: ResearchPaper[] = [
  {
    id: 'patchtst-iclr23',
    title: 'A Time Series is Worth 64 Words: Long-term Forecasting with Patch Time Series Transformer',
...
Table 2: Multivariate long-term forecasting results for H in {96, 192, 336, 720}. A lower MSE/MAE indicates better performance.`,
    mathFormulas: [
      {
        ...
        description: '将长度为 L 的单变量时间序列划分为长度为 P、步长为 S 的 N 个子序列 Patch，保留局部语义并大幅降低注意力二次复杂度。',
    ...
    experimentPlan: {
      ...
      datasets: [
        {
          ...
          sampleRate: '10 min',
          description: 'Max-Planck-Institute for Biogeochemistry weather dataset (2020), containing 21 meteorological features including temperature, humidity, pressure, wind speed.'
        },
        {
          name: 'ETTh1',
          features: 7,
      ...
      baselines: ['DLinear', 'Autoformer', 'Informer', 'FedFormer', 'PatchTST (Paper Reported)', 'PatchTST (Reproduced Ours)'],
      metrics: ['MSE', 'MAE'],
      hyperparameters: {
        seq_len: 336,
        pred_len: 96,
    ...
    paperTable2: [
      {
        ...
        h96_mse: 0.300, h96_mae: 0.384,
        h192_mse: 0.598, h192_mae: 0.544,
        h336_mse: 0.578, h336_mae: 0.523,
        h720_mse: 1.059, h720_mae: 0.741,
        avg_mse: 0.634, avg_mae: 0.548
      },
      {
        model: 'Autoformer (NeurIPS 21)',
        h96_mse: 0.266, h96_mae: 0.336,
        h192_mse: 0.307, h192_mae: 0.367,
        h336_mse: 0.359, h336_mae: 0.395,
        h720_mse: 0.419, h720_mae: 0.428,
        avg_mse: 0.338, avg_mae: 0.382
      },
      {
        model: 'FedFormer (ICML 22)',
        h96_mse: 0.217, h96_mae: 0.296,
        h192_mse: 0.276, h192_mae: 0.336,
        h336_mse: 0.339, h336_mae: 0.380,
        h720_mse: 0.403, h720_mae: 0.428,
        avg_mse: 0.309, avg_mae: 0.360
      },
      {
        model: 'DLinear (AAAI 23)',
        h96_mse: 0.176, h96_mae: 0.237,
        h192_mse: 0.220, h192_mae: 0.282,
        h336_mse: 0.265, h336_mae: 0.319,
        h720_mse: 0.323, h720_mae: 0.362,
        avg_mse: 0.246, avg_mae: 0.300
      },
      {
        model: 'PatchTST (Paper Reported)',
        isOurs: true,
        h96_mse: 0.149, h96_mae: 0.198,
        h192_mse: 0.194, h192_mae: 0.241,
        h336_mse: 0.245, h336_mae: 0.282,
        h720_mse: 0.314, h720_mae: 0.334,
        avg_mse: 0.225, avg_mae: 0.264
      }
    ],
    reproducedTable2: [
      {
        ...
        isOurs: true,
        isReproduced: true,
        h96_mse: 0.151, h96_mae: 0.199,
        h192_mse: 0.196, h192_mae: 0.243,
        h336_mse: 0.247, h336_mae: 0.285,
        h720_mse: 0.318, h720_mae: 0.338,
        avg_mse: 0.228, avg_mae: 0.266
      }
    ],
    discrepancyAnalysis: {
      overallMatchScore: 98.7,
      summary: '复现实验与原论文 Table 2 报告指标高度吻合（平均 MSE 误差仅为 +0.003，偏差率在 1.3% 以内）。各预测步长（96/192/336/720）趋势完全一致，且显著超越 DLinear 与 FedFormer 基准线。',
      metricsComparison: [
        ...
        { metric: 'Horizon 720 (MAE)', paperVal: 0.334, reproVal: 0.338, deltaPercent: 1.20, status: 'MATCH' }
      ],
      reasons: [
        {
          factor: 'GPU 硬件精度与 cuDNN 随机种子 (Floating Point Precision)',
          probability: 'High',
          explanation: '原论文在 NVIDIA A100 (40GB) 上采用 FP32 训练，不同 GPU 架构（如 V100/3090/A100）的 cuDNN 卷积与矩阵运算非确定性算法存在微量浮点误差（通常在 ±0.002 内）。',
          recommendation: '可通过固定 torch.backends.cudnn.benchmark = False 和设置多种子 (Seeds: 2021-2025) 取均值缩小差异。'
        },
        {
          factor: '可逆实例归一化 (RevIN) 的可学习参数设定',
          probability: 'Medium',
          explanation: '论文中对 Weather 数据集启用了 RevIN 仿射参数 (affine=True)，若使用静态均值方差归一化会在长序列 (H=720) 产生轻微累计误差。',
          recommendation: '已在 config.yaml 与 models/model.py 中将 RevIN(affine=True) 默认开启。'
        },
        {
          factor: '学习率 Warmup 步数与 Early Stopping 容忍度',
          probability: 'Low',
          explanation: '原作者在训练时采用了 5 epochs 的 Linear Warmup，若直接以 5e-4 学习率起始，前几个 epoch 的注意力矩阵可能会产生微小振荡。',
...
`
      },
      {
        ...
        folder: 'models',
        language: 'python',
        purpose: 'PatchTST 核心模型架构 (RevIN 归一化 + Patch Tokenizer + 通道独立 Transformer + 线性预测头)',
        content: `"""
...
`
  ...
  },
  {
    ...
    targetGoal: '按照论文 Section 5 实验设计，复现 Table 2（OGB-Arxiv 与 Cora 节点分类准确率对比）。',
    abstract: 'Graph Neural Networks (GNNs) relying on message passing suffer from over-smoothing and scalability bottlenecks. We propose NodeFormer, a scalable Graph Transformer that achieves all-pair node attention with linear time and memory complexity via kernelized Gumbel-Softmax random features.',
    sectionExcerpt: 'Section 5 Benchmark on OGB-Arxiv: Node classification with 169,343 nodes and 1.1M edges. Evaluation metric: Accuracy (%) and Macro-F1 across 10 random runs.',
    mathFormulas: [
      {
        id: 'gf1',
        ...
        description: '利用正交随机特征映射 phi(x) 将全图 O(N^2) 注意力矩阵核化为 O(N) 线性复杂度。',
      ...
      datasets: [
        {
          ...
          description: 'Open Graph Benchmark citation network with 169K computer science papers and 40 subject categories.'
        }
      ],
      forecastHorizons: [1],
      baselines: ['GCN', 'GAT', 'GraphSAGE', 'NodeFormer (Reported)', 'NodeFormer (Reproduced Ours)'],
    ...
    paperTable2: [
      {
        model: 'GCN (ICLR 17)',
        h96_mse: 71.74, h96_mae: 0.68,
        h192_mse: 0, h192_mae: 0,
        h336_mse: 0, h336_mae: 0,
        h720_mse: 0, h720_mae: 0,
        avg_mse: 71.74, avg_mae: 0.68
      },
      {
        model: 'GAT (ICLR 18)',
        h96_mse: 72.10, h96_mae: 0.69,
        h192_mse: 0, h192_mae: 0,
        h336_mse: 0, h336_mae: 0,
        h720_mse: 0, h720_mae: 0,
        avg_mse: 72.10, avg_mae: 0.69
      },
      {
        model: 'GraphSAGE (NeurIPS 17)',
        h96_mse: 72.24, h96_mae: 0.69,
        h192_mse: 0, h192_mae: 0,
        h336_mse: 0, h336_mae: 0,
        h720_mse: 0, h720_mae: 0,
        avg_mse: 72.24, avg_mae: 0.69
      },
      {
        model: 'NodeFormer (Reported)',
        isOurs: true,
        h96_mse: 73.85, h96_mae: 0.72,
        h192_mse: 0, h192_mae: 0,
        h336_mse: 0, h336_mae: 0,
        h720_mse: 0, h720_mae: 0,
        avg_mse: 73.85, avg_mae: 0.72
      }
    ],
    reproducedTable2: [
      {
        ...
        isOurs: true,
        isReproduced: true,
        h96_mse: 73.78, h96_mae: 0.72,
        h192_mse: 0, h192_mae: 0,
        h336_mse: 0, h336_mae: 0,
        h720_mse: 0, h720_mae: 0,
        avg_mse: 73.78, avg_mae: 0.72
      }
    ],
    discrepancyAnalysis: {
      overallMatchScore: 99.1,
      summary: 'OGB-Arxiv 节点分类准确率复现达到 73.78% ± 0.12%（原论文报告 73.85%），在统计置信区间 (p > 0.05) 内完全可重现。',
      metricsComparison: [
        { metric: 'ogbn-arxiv Accuracy (%)', paperVal: 73.85, reproVal: 73.78, deltaPercent: -0.09, status: 'MATCH' },
        { metric: 'Macro-F1 Score', paperVal: 0.720, reproVal: 0.719, deltaPercent: -0.14, status: 'MATCH' }
      ],
      reasons: [
        {
