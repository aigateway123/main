...
import { ScoreOptimizationStrategy } from '../types';
...
export const StepStrategy: React.FC<StepStrategyProps> = ({ strategy, onNext, onPrev }) => {
  return (
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {/* 1. 必须满足项 */}
        <div className="p-5 rounded-2xl bg-white border border-red-200 relative overflow-hidden shadow-sm">
          <div className="flex items-center gap-2 text-sm font-bold text-red-700 mb-3">
            <Flame className="w-4 h-4 text-red-600" />
            <span>1. 必须满足项（不满足一票废标）</span>
          </div>
          <ul className="space-y-2 text-xs sm:text-sm text-slate-700">
            {strategy.mustPassItems.map((item, i) => (
              <li key={i} className="flex items-start gap-2 bg-red-50/50 p-2.5 rounded-xl border border-red-100 font-medium">
        ...
        {/* 2. 核心得分项 */}
        <div className="p-5 rounded-2xl bg-white border border-blue-200 relative overflow-hidden shadow-sm">
          <div className="flex items-center gap-2 text-sm font-bold text-blue-700 mb-3">
            <Award className="w-4 h-4 text-blue-600" />
            <span>2. 核心得分项（决定最终排名的基本盘）</span>
          </div>
          <ul className="space-y-2 text-xs sm:text-sm text-slate-700">
            {strategy.coreScoringItems.map((item, i) => (
              <li key={i} className="flex items-start gap-2 bg-blue-50/50 p-2.5 rounded-xl border border-blue-100 font-medium">
        ...
        {/* 3. 容易被竞争对手拉开差距的项目 */}
        <div className="p-5 rounded-2xl bg-white border border-amber-200 relative overflow-hidden shadow-sm">
          ...
          <ul className="space-y-2 text-xs sm:text-sm text-slate-700">
            {strategy.competitiveGapItems.map((item, i) => (
              <li key={i} className="flex items-start gap-2 bg-amber-50/50 p-2.5 rounded-xl border border-amber-100 font-medium">
        ...
        {/* 4. 潜在加分项 */}
        <div className="p-5 rounded-2xl bg-white border border-emerald-200 relative overflow-hidden shadow-sm">
          <div className="flex items-center gap-2 text-sm font-bold text-emerald-800 mb-3">
            <PlusCircle className="w-4 h-4 text-emerald-600" />
            <span>4. 潜在加分项（补充材料可拿额外分）</span>
          </div>
          <ul className="space-y-2 text-xs sm:text-sm text-slate-700">
            {strategy.bonusItems.map((item, i) => (
              <li key={i} className="flex items-start gap-2 bg-emerald-50/50 p-2.5 rounded-xl border border-emerald-100 font-medium">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-5 sm:p-6 shadow-sm">
        ...
        <div className="space-y-3.5">
          {strategy.actionableTactics.map((tactic, idx) => (
            <div
              key={idx}
              className="p-4 sm:p-5 rounded-xl bg-slate-50 border border-slate-200 hover:border-slate-300 transition-colors"
            >
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-2">
                <div className="flex items-center gap-2">
                  <span className="w-6 h-6 rounded-full bg-blue-100 text-blue-700 font-bold text-xs flex items-center justify-center font-mono border border-blue-200">
                    {idx + 1}
                  </span>
