...
// 轻量 CSS 粒子礼花层（替代原型 canvas-confetti，零依赖）
// 用法：父组件 <EcomConfettiLayer :trigger="on" @done="on = false" />，:trigger 置 true 即喷射 1.7s
...
const COLORS = ['#6366f1', '#8b5cf6', '#a855f7', '#ec4899', '#f59e0b', '#10b981', '#06b6d4', '#f472b6']
...
<template>
  <div v-if="active" class="pointer-events-none fixed inset-0 z-[90] overflow-hidden">
    <span
      v-for="p in pieces"
      :key="p.id"
      class="ecom-confetti-piece absolute -top-3"
      :style="{
        left: p.left,
...
<style scoped>
.ecom-confetti-piece {
  animation-name: ecom-confetti-fall;
  animation-timing-function: linear;
  animation-iteration-count: 1;
  animation-fill-mode: forwards;
}
@keyframes ecom-confetti-fall {
  0% {
