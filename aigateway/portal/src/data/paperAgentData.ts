// ============================================================================
// Paper Agent —— 从实验结果到论文初稿 演示数据
//
// 还原自外部参考 demo「ai-paper-agent」：
// - 预置科研实验项目（ST-Trans 城市 EV 快充负荷预测）
// - AI 自动生成的论文初稿（IEEE 格式全文）
// - 消融实验数据 + AI 同行审稿报告（Major / Minor 问题闭环）
// ============================================================================
  ...
  figures: [
    {
      ...
      caption:
        '图 1. 本文提出的 ST-Transformer、基准模型 LSTM 与全城充电站实际观测真实负荷（含 95% 置信区间）的 24 小时预测拟合对比。',
      type: 'line_chart',
      dataDescription:
        '展示 24 小时负荷动态轨迹，尤其在 18:00–21:00 晚高峰及电价波峰时段，ST-Transformer 展现出优异的拟合精度与极低相位滞后。',
    },
    {
      id: 'fig-2',
      number: 2,
      ...
      caption:
        '图 2. 8 个核心充电枢纽区域之间的空间注意力权重热力图，清晰揭示了动态电价波峰时段车辆从市中心向周边副中心跨区充电迁移的行为模式。',
      type: 'heatmap',
      dataDescription:
        '跨站点交叉注意力权重矩阵，展现市中心快充站与郊区枢纽站之间因分时电价差异驱动的动态负荷转移关联。',
    },
  ],
  tables: [
    {
...
export const INITIAL_REVIEW_REPORT: ReviewReport = {
  ...
  summary:
    '本论文针对动态分时电价下城市电动汽车快充负荷多步时空预测这一兼具学术价值与工程意义的问题展开研究。所提出的 Transformer 架构构想清晰，初步定量结果令人满意。然而，当前版本在实验严密性与学术规范上存在若干重大缺陷：论文缺乏必不可少的消融实验（Ablation Study）来系统论证各多模态特征与核心机制的独立贡献度，缺乏多随机种子的统计显著性检验，且在图表置信区间与量纲单位上存在细节不足，因此暂不能直接录用，建议大修。',
  strengths: [
  ...
  majorIssues: [
    {
      ...
      critique:
        '作者声称动态电价、气象特征与空间动态交叉注意力共同带来了精度的提升，但第 4 节中没有任何消融实验（Ablation Study）数据支撑！审稿人无法判断性能增益究竟来自深层 Transformer 本身，还是来自特定领域外生特征。',
...
用于定量验证不同特征与核心机制对模型整体性能（MAE / RMSE / MAPE）的具体贡献度。`,
    ...
    },
    {
      ...
      critique:
        '表 2 中的评估指标仅为单次运行的点估计值，未给出 5 折交叉验证或多次随机种子的标准差（Mean ± Std），且缺少配对 t-检验 (Student t-test) 的 p-value 检验结果。',
  ...
  minorIssues: [
    {
      ...
      critique:
        'Figure 1 的负荷拟合折线图仅绘制了单条确定性曲线，缺少展示站点间波动与峰值不确定性的 95% 置信区间阴影带。',
